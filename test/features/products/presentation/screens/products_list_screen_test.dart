...
class _StubListNotifier extends ProductsListNotifier {
  _StubListNotifier(super.repository, ProductsListState initialState) {
    state = initialState;
  }
...
class _DummyRepository implements ProductRepository {
  @override
      ...
      await buildTestApp(
        overrides: [
          productsListProvider.overrideWith(
            (ref) => _StubListNotifier(
              _DummyRepository(),
              const ProductsListState(isLoading: true),
            ),
          ),
        ],
        child: const TestMaterialApp(home: ProductsListScreen()),
      ),
    );
    await tester.pump();
  ...
  testWidgets('shows empty state when no products', (tester) async {
    await tester.pumpWidget(
      await buildTestApp(
        overrides: [
          productsListProvider.overrideWith(
            (ref) => _StubListNotifier(_DummyRepository(), const ProductsListState()),
          ),
        ],
        child: const TestMaterialApp(home: ProductsListScreen()),
      ),
